...
for select
to authenticated
using (public.current_app_user_can_sell_at_location(location_id));

create policy "Users can update inventory for selling at allowed stores"
on public.inventory
for update
to authenticated
using (public.current_app_user_can_sell_at_location(location_id))
with check (public.current_app_user_can_sell_at_location(location_id));

create policy "Users can read sale movements at allowed stores"
