...
using (
  ...
  and (
    (reason = 'receive' and public.current_app_user_can_view_receiving_at_location(location_id))
    or (reason = 'sale' and public.current_app_user_can_view_sales_at_location(location_id))
    or (reason in ('NEW_ITEM', 'MANUAL_ADJUSTMENT') and public.current_app_user_can_view_inventory())
    or (reason not in ('receive', 'sale', 'NEW_ITEM', 'MANUAL_ADJUSTMENT') and public.current_app_user_can_view_inventory())
  )
);
...
with check (
  public.current_app_user_has_location(location_id)
  and (
    (reason = 'receive' and public.current_app_user_can_receive_at_location(location_id))
    or (reason = 'sale' and public.current_app_user_can_sell_at_location(location_id))
