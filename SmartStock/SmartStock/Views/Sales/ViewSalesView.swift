    ...
    private func loadSales() async {
        ...
        do {
            sales = try await supabase
                .from("sales")
                .select("sale_id, subtotal_amount, total_amount, discount_amount, status, payment_method, payment_reference, transaction_source, created_at, payment_status, amount_paid, returned_amount, receipt_number, receipt_device_id, receipt_sequence, users(full_name), locations(name), customer_accounts(name), sale_items(quantity)")
